...
// ── EXCLUSIONES ──
// Single source for "which hours count as billable" - previously
// reimplemented three times (this file, lib/report.js, lib/throughput.js),
// with this file's version silently excluding "Event Support - LA" and the
// other two not. report.js/throughput.js's narrower list is the one that
// actually feeds the live Week View, Dashboard, and daily email, so THAT
// list is now canonical here too - this file's behavior changes (drops the
// Event Support - LA exclusion) so the demo endpoints that call
// aggregatePayroll() below start agreeing with the live report instead of
// diverging from it. report.js and throughput.js now import this function
...
// excluded: Set de nombres ya normalizados (asi los construyen report.js y
// throughput.js) o array de nombres crudos (asi los da lib/data.js). Se
// soportan ambas formas para no forzar un cambio en cada quien construye la
// lista, solo en la regla de exclusion en si.
export function isExcludedEmp(name, excluded) {
  if (!excluded) return false;
  const n = normEmp(name);
  if (typeof excluded.has === "function") return excluded.has(n);
  return excluded.some((e) => normEmp(e) === n);
}
...
// jobTitle: crudo (sin normalizar) - esta funcion normaliza internamente.
export function exclusionReason(empName, jobTitle, excluded) {
  if (empName && isExcludedEmp(empName, excluded)) return "Excluded list";
  const jtN = normJobTitle(jobTitle);
...
export function aggregatePayroll(rows, excludedList) {
  ...
  rows.forEach((r) => {
    ...
    if (total <= 0) return;
    const excl = exclusionReason(empName, r.jobTitle, excludedList);
