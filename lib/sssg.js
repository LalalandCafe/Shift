...
// UNIFORMITY IS A REQUIREMENT, NOT AN ACCIDENT. Finance decided 2026-09-16
// that SHIFT publishes its own computed figure for every period, with no UI
// marker and no dual display, even for the periods whose workbook totals were
// typed in by hand rather than calculated (P12 2025 and P2-P8 2026 - see
// docs/finance/README.md, "Workbook figure provenance"). The consequence for
// this file: there must be no per-period, per-month or per-year branch
// anywhere in it. No period is excluded, corrected, annotated or
// special-cased because its workbook counterpart disagrees.
//
// If a SHIFT number differs from a figure Finance circulated, that gets
// explained when it is asked about. It does not get patched here. A single
// `if (period === ...)` in this file would be indistinguishable from a bug
// six months later, and it would make every other period's number suspect.
//
// Verified 2026-09-16: no such branch exists. The only thresholds here are
// MIN_COMPARABLE_STORES and the data-driven rules in classifyStore, both of
// which apply identically to every period.
 ...
 *
 * NOTE ON opened_at. This rule deliberately does NOT read stores.opened_at -
 * it infers "not open yet" from the prior-year month being all $0/null, so it
 * stays correct even when opened_at is wrong. But opened_at still governs
 * whether the rows exist at all: backfill-sales.yml skips any store whose
 * opened_at is NULL or later than the date being synced. A new store added
 * without opened_at is therefore never backfilled, lands here with no rows,
 * and is excluded as "incomplete data" - silently, and for a reason that has
 * nothing to do with the real one. 10033 Melrose is the next store this will
 * apply to. See docs/finance/README.md.
 *
