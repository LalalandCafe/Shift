...
// Command Center Phase 3 (accountability). gm_name/area_manager_name are
// manually maintained the same way grp is - not sourced from Toast, which
// has no stable per-store manager signal (see
// docs/plans/FEATURE-PLAN-2026-09-04.md, feature 6). Requires
// docs/sql/001-store-managers.sql to have been run; until then this
// throws the same "column does not exist" error Postgres always gives for
// an unmigrated column, surfaced to the caller rather than swallowed.
export async function updateStoreManagers(supabase, code, gmName, areaManagerName) {
  const { error } = await supabase
    .from("stores")
    .update({
      gm_name: gmName || null,
