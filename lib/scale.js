 ...
 * where the margin would put it. Drive-thru is the case: target 2:15 (135s),
 * with an explicit green line at 2:05 (125s) and red at 2:30 (150s).
 *
 * Because dt_window sets green_value, it does NOT use the COMFORT margin at
 * all. The margin would have put green at 1:55 (0.85 x 135s); operations
 * wanted 2:05, which is 93% of target rather than 85%.
 *
 ...
 *
 * Footgun worth knowing before you write this column: the
 * metric_targets_direction CHECK constraint validates red_value against
 * target_value only. It does NOT validate green_value. A green that sits on
 * the wrong side of target passes the constraint and then silently collapses
 * the light-green band to nothing, because bandFor() tests v <= green before
 * it tests v <= target, so every value that would have been light green is
 * claimed by green first. Postgres will not catch that ordering for you.
 * Check it by hand.
 */
