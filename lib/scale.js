...
/**
 * Three-state goal status for Command Center's goal chips - distinct from
 * bandFor's four-band WeekView coloring, which always needs a redLine to
 * classify anything (see cfgFromTarget above). A goal chip is simpler and
 * has to work for a metric that only has a target and no red-line yet
 * (e.g. SSSG's 0% target, TPLH before it has one): "onGoal" or "offGoal"
 * are always answerable from target alone, "action" only exists once a
 * redLine is actually supplied - never invented from the target.
 *
 * @param {number|null} value
 * @param {object} cfg  same shape as bandFor's cfg; cfg.redLine may be null
 * @returns {"onGoal"|"offGoal"|"action"|"none"}
 */
export function goalStatus(value, cfg) {
  const v = num(value);
  if (v === null || !cfg || cfg.target == null) return "none";
  const { target, redLine, lowerIsBetter = true } = cfg;
  const onGoal = lowerIsBetter ? v <= target : v >= target;
  if (onGoal) return "onGoal";
  if (redLine == null) return "offGoal";
  const pastRedLine = lowerIsBetter ? v > redLine : v < redLine;
  return pastRedLine ? "action" : "offGoal";
}
...
export function cfgFromTarget(row) {
  ...
  return {
    ...
    // A metric can have a target with no red-line yet (nobody has set one -
    // see docs/sql/003-unify-metric-targets.sql). Number(null) is 0, not
    // null, which would silently read as "the red line is zero" instead of
    // "there is no red line" - that's wrong for a higher-is-better metric
    // especially, where a store missing its target would then almost never
    // reach the real red band. Mirrors the green_value null-check below.
    redLine: row.red_value == null ? null : Number(row.red_value),
...
export function fmtForUnit(unit) {
  ...
  // percent/dollars added for Command Center's goal chips (SSSG, SPLH) -
  // every unit on metric_targets before this was "seconds", so nothing
  // existing depended on the fallback being wrong for these.
  if (unit === "percent") return (n) => `${Number(n) > 0 ? "+" : ""}${Number(n).toFixed(1)}%`;
  if (unit === "dollars") return (n) => `$${Number(n).toFixed(2)}`;
  return fmtSeconds;
}
