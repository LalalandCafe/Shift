...
// Lee toast_labor_shifts para un rango, aplica exclusiones, y devuelve
// tanto los totales como el desglose por dia (para el WTD Over/Under).
async function readLabor(startIso, endIso, excludedSet) {
  const rows = await fetchAllRows(
    (from, to) =>
      supabaseAdmin
        .from("toast_labor_shifts")
        .select("store_id, hours, job_title, employee_name, clock_in")
  ...
  const perDay = {}; // perDay[code][isoDate] = hours
  ...
  return { hours, trainee, trainer, perDay, rowCount: rows.length };
}
...
async function sumSalesByStore(startIso, endIso) {
  ...
  const map = {};
  const perDay = {}; // perDay[code][isoDate] = sales
  rows.forEach((r) => {
...
export async function buildDailyReport(isoDate) {
  ...
  const dayLabor = await readLabor(isoDate, isoDate, excludedSet);
  const { map: daySales } = await sumSalesByStore(isoDate, isoDate);

  const wtdLabor = await readLabor(weekStart, isoDate, excludedSet);
  ...
  // Dias de la semana hasta la fecha del reporte, para el over/under dia-por-dia
  ...
  if (periodInfo && isoDate > FILE_COVERS_THROUGH) {
    ...
    const extraLabor = await readLabor(extraStart, isoDate, excludedSet);
  ...
  const rows = stores.map((st) => {
    ...
    // WTD Over/Under: suma dia por dia usando el target de cada dia,
    // igual que la offline tool (no usa el target del dia del reporte
    // para toda la semana).
    let wOverUnder = 0;
