...
// Pagina cualquier consulta. buildQuery DEBE construir un query nuevo en cada
// llamada; los builders de Supabase son de un solo uso y no se pueden reutilizar.
...
// Una sola lectura de toast_labor_shifts que devuelve horas totales y training.
async function readLabor(startIso, endIso) {
  const rows = await fetchAllRows(
    (from, to) =>
      supabaseAdmin
        .from("toast_labor_shifts")
        .select("store_id, hours, job_title")
  ...
  return { hours, trainee, trainer, rowCount: rows.length };
}
...
export async function buildDailyReport(isoDate) {
  ...
  const dayLabor = await readLabor(isoDate, isoDate);
  const daySales = await sumSalesByStore(isoDate, isoDate);

  const wtdLabor = await readLabor(weekStart, isoDate);
  ...
  if (periodInfo && isoDate > FILE_COVERS_THROUGH) {
    ...
    const extraLabor = await readLabor(extraStart, isoDate);
  ...
  const rows = stores.map((st) => {
    ...
    const wOverUnder = dTarget > 0 ? (wS / dTarget) - wH : 0;
