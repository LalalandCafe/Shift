...
async function sumTrainingByStore(startIso, endIso) {
  const { data, error } = await supabaseAdmin
    .from("toast_labor_shifts")
    .select("store_id, hours, job_title, clock_in")
    .gte("clock_in", startIso + "T00:00:00")
    .lte("clock_in", endIso + "T23:59:59");
  if (error) throw new Error("sumTraining: " + error.message);
  const trainee = {}, trainer = {};
  (data || []).forEach((r) => {
    const code = parseInt(r.store_id);
    const jt = (r.job_title || "").replace(/\*$/, "").trim().toLowerCase();
    if (jt === "trainee") trainee[code] = (trainee[code] || 0) + (r.hours || 0);
    if (jt === "certified trainer") trainer[code] = (trainer[code] || 0) + (r.hours || 0);
  });
...
export async function buildDailyReport(isoDate) {
  ...
  const wtdTraining = await sumTrainingByStore(weekStart, isoDate);
  ...
  const rows = stores.map((st) => {
    ...
    const wTrainee = (wtdTraining.trainee[code] || 0);
    const wTrainer = (wtdTraining.trainer[code] || 0);
