 ...
 *
 * KNOWN, UNFIXABLE-FROM-HERE GAP, and it is one-directional. A refund whose
 * refundBusinessDate differs from its order's businessDate is counted on
 * NEITHER day: skipped here (wrong date), and never seen on the refund's own
 * day either, because that day's ordersBulk pull returns orders by ORDER
 * date and this order belongs to a different one. Those refunds are missing
 * from net entirely, which always overstates net, never understates it. They
 * are reported in `skippedOtherDate` rather than swallowed, so the size of
 * that bias is measurable from our own logs instead of being an unknown.
 *
